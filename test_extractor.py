...
# ใบจริง B2S — หน้า 58 ของกอง "ใบจริงอันใหม่.pdf" และเป็นใบที่ผู้ใช้อัปโหลด
# เข้าเว็บจริงแล้วอ่านผิด 5 ช่องรวด
#
# ที่มาของข้อความ: ประกอบขึ้นจากภาพเอกสาร ไม่ใช่ข้อความที่ Cloud Vision
# คายออกมาจริง (ยังไม่ได้ข้อความดิบของใบนี้) แต่ยืนยันได้ว่าใกล้พอ:
# เอาข้อความนี้ป้อนให้โค้ดตัวที่อยู่บนเว็บ ได้ผลตรงกับหน้าจอที่ผู้ใช้เห็น
# ทุกช่อง — ประเภท=ย่อ, ผู้ซื้อ=ว่าง, ยอดก่อนภาษี/VAT/ยอดรวม=ว่างทั้งสาม
#
# สิ่งที่ใบนี้ทำให้พัง มีสี่อย่างซ้อนกัน:
#   1. กล่องลูกค้าใช้ป้ายว่า "ชื่อ" กับ "ที่อยู่" เท่านั้น ไม่มีคำว่า
#      ผู้ซื้อ/ลูกค้า/Customer -> ชื่อผู้ซื้อว่าง -> ถูกจัดเป็นใบย่อ ->
#      ยอดก่อนภาษีกับ VAT ถูกล้างทิ้งตาม ม.86/6
#   2. "มูลค่าสินค้า" ไปตรงกับบรรทัด "...ที่ยกเว้นภาษีมูลค่าเพิ่ม" (0.00)
#      ซึ่งอยู่เหนือบรรทัดยอดก่อนภาษีจริง
#   3. "...ที่รวมภาษีมูลค่าเพิ่ม" (52.00 = ยอดรวม) ไปตรงกับคำค้น VAT
#   4. เลขใต้บาร์โค้ด 23 หลักถูกหยิบเป็นจำนวนเงิน
...
ชื่อ คณะบริหารธุรกิจ มหาวิทยาลัยเชียงใหม่ สาขาที่ 00120
ที่อยู่ เลขที่ 239 ถนน ห้วยแก้ว ตำบล สุเทพ อำเภอ เมืองเชียงใหม่ จังหวัด เชียงใหม่ 50200
ลำดับที่ รหัสสินค้า ชื่อสินค้า จำนวน ราคา จำนวนเงิน
1 V 8850968601113 SBกระดาษปรู๊ฟขาว P10 1.00 52.00 52.00
TP No. 103-115862
66202507225005110311586
รวม 1 52.00
ส่วนลด : 0.00
(ห้าสิบสองบาทถ้วน) รวมเงิน : 52.00
ชำระโดย
- มูลค่าสินค้าที่ยกเว้นภาษีมูลค่าเพิ่ม : 0.00 QRPP 114100XXXXXX6 : 52.00
- มูลค่าสินค้าที่รวมภาษีมูลค่าเพิ่ม : 52.00 Change : 0.00
- มูลค่าสินค้าที่เสียภาษีมูลค่าเพิ่ม : 48.60
- ภาษีมูลค่าเพิ่ม : 3.40
...
"""
    ...
    all_ok &= check("real b2s: buyer name (label is the bare word ชื่อ)",
                    b2s["buyer_name"] == "คณะบริหารธุรกิจ มหาวิทยาลัยเชียงใหม่ สาขาที่ 00120")
    ...
    all_ok &= check("real b2s: vat is 3.40, not the VAT-inclusive 52.00",
                    b2s["vat"] == 3.40)
    all_ok &= check("real b2s: total", b2s["total"] == 52.00)
