...
# ใบจริง ร้านสหกรณ์มหาวิทยาลัยเชียงใหม่ — หน้า 6 (และซ้ำที่หน้า 54) ของกอง
# "ใบจริงอันใหม่.pdf" ข้อความดิบจาก Cloud Vision
#
# สองช่องที่ผิด มาจากคนละสาเหตุกันคนละเรื่อง:
#
#   เลขที่ใบกำกับภาษี = "0.00"
#     ป้าย "เลขที่เอกสาร P004000599" พิมพ์ค่าไว้บนบรรทัดเดียวกันอยู่แล้ว
#     และการค้นแบบปกติก็หาเจอถูกตั้งแต่แรก แต่ตัวจับคู่ป้ายกับแถวค่า
#     (ซึ่งมีสิทธิ์เหนือกว่า) ไปจับป้ายนี้เข้ากับแถวตัวเลขของกล่องยอด
#     ด้านล่าง ได้ "0.00" มาทับของที่ถูกอยู่แล้ว
#
#   วันที่ = 2015-07-14 (ใบพิมพ์ 2025)
#     ใบนี้มีวันที่สองที่ "วันที่ (Date) : 14/07/2015" ซึ่ง OCR อ่านปีผิด
#     และ "วันที่ใบกำกับภาษี" ในแถวหัวตารางซึ่งค่าถูกต้องคือ 14/07/2025
#     ป้ายอันหลังเจาะจงกว่าและควรชนะ แต่ถูกตัดทิ้งเพราะบรรทัดหัวตารางยาว
#     เกินเกณฑ์ "ร้อยแก้ว" — ยาวเพราะมีหัวคอลัมน์อีกหกอันต่อท้าย
#     ผิดไปสิบปีแปลว่ายื่น ภ.พ.30 คนละรอบ
REAL_CMUCOOP_RAW_TEXT = """ร้านสหกรณ์มหาวิทยาลัยเชียงใหม่ จำกัด
239 ถนนห้วยแก้ว ตำบลสุเทพ อำเภอเมืองเชียงใหม่ จังหวัดเชียงใหม่ 50200
เลขประจำตัวผู้เสียภาษี : 0994000433816
สาขาที่ออกใบกำกับภาษี :สาขา SW CMUCO-OP-MKT
ชื่อลูกค้า!
หหาวิทยาลัยเชียงใหม่
เลขประจำตัวผู้เสียภาษี : 0994000423179
ที่อยู่ : 239 ถนนห้วยแก้ว ตำบลสุเทพ อำเภอเมือง จังหวัดเชียงใหม่ 50200
ใบกำกับภาษี/ใบเสร็จรับเงิน
เลขที่เอกสาร P004000599
วันที่ (Date) : 14/07/2015
โทรศัพท์ 053943105
โทรสาร
วันที่ใบกำกับภาษี เวลา (Time) ลำดับเครื่องกับเงิน หมายเลขเครื่องเก็บเงิน เลขที่ใบกำกับภาษีอย่างย่อ พนักงานเก็บเงิน แผ่นที่
151
14/07/2025
14:15:51
005
...
รายการ (Description)
IV 30001758
กระบอกน้ำสแตนเลส mL.V1300- ขาว
2 V 30001777
ยาดมสมุนไพรหอม 20g. กระปุกสีม่วง
0001615
จำนวน ราคา/หน่วย
ส่วนลด
ราคาสุทธิ
...
36.18
553.00
จำนวนเงินก่อนภาษีมูลค่าเพิ่ม
ภาษีมูลค่าเพิ่ม 7%
จำนวนเงินรวมกาฟันเล่าเป็น
ฐานสหกรณ์มหาวิทยาลัยเชียงใหม่,
ลงชื่อ รา
ผู้รับเงิน
"""
...
"""
    ...
    # ---- ใบจริง ร้านสหกรณ์ มช. ----
    coop = extractor.extract_fields(REAL_CMUCOOP_RAW_TEXT, ocr_confidence=90.0)
    all_ok &= check("real cmucoop: invoice_no is the document number, not an amount",
                    coop["invoice_no"] == "P004000599")
    all_ok &= check("real cmucoop: date comes from the specific label in the header row",
                    coop["invoice_date_iso"] == "2025-07-14")
    all_ok &= check("real cmucoop: seller",
                    coop["seller_name"] == "ร้านสหกรณ์มหาวิทยาลัยเชียงใหม่ จำกัด")
    all_ok &= check("real cmucoop: seller tax id", coop["seller_tax_id"] == "0994000433816")
    all_ok &= check("real cmucoop: subtotal", coop["subtotal"] == 516.82)
    all_ok &= check("real cmucoop: vat", coop["vat"] == 36.18)
    all_ok &= check("real cmucoop: total", coop["total"] == 553.00)
    all_ok &= check("real cmucoop: เต็มรูป", coop["doc_type"] == "เต็มรูป")
    # ผู้ซื้อบนใบจริงคือ "คณะบริหารธุรกิจ มหาวิทยาลัยเชียงใหม่" แต่ OCR ทำ
    # คำว่า "คณะบริหารธุรกิจ" หายไปทั้งคำ เหลือแค่ "หหาวิทยาลัยเชียงใหม่"
    # (ม กลายเป็น ห ด้วย) สิ่งที่ถูกต้องที่สุดที่ทำได้จากข้อความนี้คือคืน
    # สิ่งที่อยู่บนกระดาษ ไม่ใช่เติมคำที่ไม่มีให้เอง
    all_ok &= check("real cmucoop: buyer is what OCR actually read, not an invention",
                    coop["buyer_name"] == "หหาวิทยาลัยเชียงใหม่")
    # การ์ด: จำนวนเงินไม่ใช่เลขเอกสาร แต่เลขล้วน 12 หลักยังเป็นได้
    all_ok &= check("an amount cannot validate a document-info pairing",
                    not extractor._doc_info_pairing_is_sound({"doc_no": "0.00"}))
    all_ok &= check("a digits-only POS receipt number still can",
                    extractor._doc_info_pairing_is_sound({"doc_no": "041501408013"}))
